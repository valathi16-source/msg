...
import { X, Search, UserPlus, Phone, UserCheck } from 'lucide-react';
...
export default function UserListModal({ currentUser, onClose, onSelectChat }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-[#1f2c34] border border-gray-700/60 rounded-2xl shadow-2xl overflow-hidden text-gray-100 flex flex-col max-h-[85vh]">
        ...
        {/* Search Input */}
        <div className="p-3 bg-[#111b21]/50 border-b border-gray-800">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-[#111b21] border border-gray-700 rounded-lg text-sm text-white focus:outline-none focus:border-emerald-500"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {loading ? (
            <div className="p-6 text-center text-xs text-gray-400">Loading users...</div>
          ) : filteredUsers.length === 0 ? (
            <div className="p-6 text-center text-xs text-gray-400">
              No registered users found. Open another browser session to create a user or guest!
            </div>
          ) : (
            filteredUsers.map((user) => (
              <button
                ...
                className="w-full p-3 flex items-center gap-3 rounded-xl hover:bg-[#2a3942] transition-all text-left group"
              >
